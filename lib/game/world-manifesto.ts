...
export function getWorldManifestoPrompt(context?: string): string {
  return `
...
export function validateActionAgainstManifesto(action: string, context: string): boolean {
  ...
  return manifestoKeywords.some(keyword =>
    actionLower.includes(keyword) || contextLower.includes(keyword)
  )
}