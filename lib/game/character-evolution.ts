...
export enum CreatureType {
  ...
  ENTIDADES_CONTATO = 'entidades_contato',
  GHOULS = 'ghouls',
  YOKAI_TRADICIONAIS = 'yokai_tradicionais',
  YUREI = 'yurei',
  MONONOKE = 'mononoke',
  KAMI_MENORES = 'kami_menores',
...
export enum CharacterClass {
  ...
  MONGE_BUDISTA = 'monge_budista',
  SACERDOTE_XINTOISTA = 'sacerdote_xintoista',
  SOLDADO_RASO = 'soldado_raso',
  CURANDEIRO = 'curandeiro',
  ARTISTA_AMBULANTE = 'artista_ambulante',

  // Classes evoluídas baseadas em criaturas
  GUARDIAO_DOS_IMPOSTORES = 'guardião_dos_impostores',     // Substitutos
  VIAJANTE_ENTRE_MUNDOS = 'viajante_entre_mundos',        // Entidades de Contato
  DEVORADOR_DE_ALMAS = 'devorador_de_almas',              // Ghouls
  MEDIADOR_ESPIRITUAL = 'mediador_espiritual',            // Yōkai Tradicionais
  GUIA_DOS_PERDIDOS = 'guia_dos_perdidos',                 // Yūrei
  HARMONIZADOR_DE_EMOCOES = 'harmonizador_de_emocoes',     // Mononoke
  PROTETOR_DA_NATUREZA = 'protetor_da_natureza',           // Kami Menores
  GUARDIAO_DOS_ANTIGOS = 'guardião_dos_antigos'            // Tsukumogami
}

// Atributos expandidos incluindo espirituais
export interface ExpandedAttributes {
  ...
  // Atributos espirituais (despertados através de encontros)
  harmonia_espiritual: number    // Capacidade de se conectar com espíritos
  resistencia_sobrenatural: number // Resistência a influências espirituais
  afinidade_elemental: number    // Controle sobre elementos naturais
  empatia_emocional: number      // Capacidade de sentir e influenciar emoções
  memoria_ancestral: number      // Conhecimento de eventos passados
  adaptabilidade_mistica: number // Capacidade de se adaptar a situações sobrenaturais

  // Atributos especializados (desbloqueados por classes)
  furtividade_sombria?: number   // Para Guardião dos Impostores
  clarividencia?: number         // Para Viajante entre Mundos
  voracidade_controlada?: number // Para Devorador de Almas
  diplomacia_espiritual?: number // Para Mediador Espiritual
  empatia_pós_vida?: number      // Para Guia dos Perdidos
  harmonizacao_emocional?: number // Para Harmonizador de Emoções
  comunhao_natural?: number      // Para Protetor da Natureza
  preservacao_temporal?: number  // Para Guardião dos Antigos
}

export interface CreatureEncounterRecord {
...
export interface CharacterEvolution {
  ...
  creatureEncounters: Record<CreatureType, CreatureEncounterRecord>
  evolutionPoints: number
  spiritualAwakening: number // 0-100, quanto o personagem está "desperto"
  classMastery: Record<CharacterClass, number> // 0-100 por classe
  uniqueTraits: string[] // Traços únicos adquiridos
  evolutionHistory: Array<{
    date: Date
    event: string
    classChange?: CharacterClass
    attributeChanges: Partial<ExpandedAttributes>
  }>
}
...
export interface Family {
  ...
  founder: string // ID do personagem fundador
  ...
  familyTraits: string[] // Traços herdados pela família
  bloodlinePowers: string[] // Poderes especiais da linhagem
  rivalFamilies: string[] // IDs de famílias rivais
  alliances: string[] // IDs de famílias aliadas
}

export class CharacterEvolutionManager {
  private evolutionData: CharacterEvolution

  constructor(initialClass: CharacterClass = CharacterClass.CAMPONES) {
    this.evolutionData = {
      currentClass: initialClass,
      availableClasses: [initialClass],
      creatureEncounters: {} as Record<CreatureType, CreatureEncounterRecord>,
      evolutionPoints: 0,
  ...
  recordCreatureEncounter(
    ...
    if (success) {
      ...
      if (lesson) record.learnedLessons.push(lesson)
      if (grantedPower) record.grantedPowers.push(grantedPower)
    } else {
    ...
    this.evolutionData.spiritualAwakening = Math.min(100,
      this.evolutionData.spiritualAwakening + (success ? 2 : 1)
    )
  ...
  private checkClassUnlock(creatureType: CreatureType): void {
    const record = this.evolutionData.creatureEncounters[creatureType]
    const classUnlocks: Record<CreatureType, { class: CharacterClass, requiredEncounters: number }> = {
  ...
  changeClass(newClass: CharacterClass): boolean {
    ...
    this.evolutionData.classMastery[newClass] = Math.min(100,
      (this.evolutionData.classMastery[newClass] || 0) + 10
    )
  ...
  private getClassAttributeBonuses(characterClass: CharacterClass): Partial<ExpandedAttributes> {
    const bonuses: Record<CharacterClass, Partial<ExpandedAttributes>> = {
  ...
  getEvolutionData(): CharacterEvolution {
    return this.evolutionData
  }

  // Calcular atributos totais incluindo bônus de classe e criaturas
  calculateTotalAttributes(baseAttributes: ExpandedAttributes): ExpandedAttributes {
  ...
  private getCreatureAttributeBonuses(creatureType: CreatureType, successfulEncounters: number): Partial<ExpandedAttributes> {
    const baseBonus = Math.floor(successfulEncounters / 2) // 1 ponto a cada 2 encontros bem-sucedidos

    const bonuses: Record<CreatureType, Partial<ExpandedAttributes>> = {
      [CreatureType.SUBSTITUTOS]: {
  ...
  deserialize(data: string): void {
    try {
      ...
      Object.values(this.evolutionData.creatureEncounters).forEach((record: any) => {
        record.lastEncounter = new Date(record.lastEncounter)
      })

      this.evolutionData.evolutionHistory.forEach((entry: any) => {
