...
exports.mapManager = exports.MapManager = exports.JAPAN_FEUDAL_MAP = void 0;
// Mapa completo do Japão feudal - Estrutura geográfica
exports.JAPAN_FEUDAL_MAP = {
    // === PROVÍNCIA DE OWARI ===
    owari_village: {
        id: 'owari_village',
...
class MapManager {
    ...
    getPlayersInArea(locationId, radius = 50) {
        const location = exports.JAPAN_FEUDAL_MAP[locationId];
        if (!location)
            return [];
    ...
    findPath(startLocationId, endLocationId) {
        ...
        const start = exports.JAPAN_FEUDAL_MAP[startLocationId];
        const end = exports.JAPAN_FEUDAL_MAP[endLocationId];
        if (!start || !end)
            return [];
        ...
        while (queue.length > 0) {
            ...
            const currentLocation = exports.JAPAN_FEUDAL_MAP[current.locationId];
    ...
    getNearbyLocations(coordinates, maxDistance = 100) {
        return Object.values(exports.JAPAN_FEUDAL_MAP)
    ...
    isLocationAccessible(fromLocationId, toLocationId) {
        const path = this.findPath(fromLocationId, toLocationId);
        return path.length > 0;
    }
    // Obter estatísticas da área
    getAreaStats(locationId, radius = 100) {
        const nearbyLocations = this.getNearbyLocations(exports.JAPAN_FEUDAL_MAP[locationId]?.coordinates || { x: 0, y: 0 }, radius);
        ...
        const avgDanger = nearbyLocations.reduce((sum, loc) => sum + dangerLevels[loc.danger], 0) / nearbyLocations.length;
        let dangerLevel;
        if (avgDanger < 0.5)
            dangerLevel = 'safe';
        else if (avgDanger < 1.5)
            dangerLevel = 'moderate';
        else if (avgDanger < 2.5)
            dangerLevel = 'dangerous';
        else
            dangerLevel = 'extreme';
        // Calcular presença sobrenatural média
        const avgSupernatural = nearbyLocations.reduce((sum, loc) => sum + loc.supernaturalPresence, 0) / nearbyLocations.length;
...
exports.mapManager = new MapManager();
