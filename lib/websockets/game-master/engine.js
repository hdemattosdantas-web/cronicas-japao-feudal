...
exports.gameMaster = exports.GameMasterEngine = void 0;
const openai_service_1 = require("../ai/openai-service");
const creature_generator_1 = require("../ai/creature-generator");
const game_master_personality_1 = require("../ai/game-master-personality");
const game_master_memory_1 = require("../ai/game-master-memory");
const world_manifesto_1 = require("../game/world-manifesto");
const creature_types_1 = require("../game/creature-types");
class GameMasterEngine {
    constructor() {
        this.ai = openai_service_1.openaiService;
        this.creatureGenerator = new creature_generator_1.CreatureGenerator();
        this.personalityManager = game_master_personality_1.personalityManager;
        this.memory = game_master_memory_1.gameMasterMemory;
    }
    async processPlayerAction(gameState, currentScene, action, choiceId) {
        ...
        this.personalityManager.adaptPersonality({
            ...
            supernaturalEvents: gameState.choicesMade.filter(choice => choice.includes('creature') || choice.includes('supernatural')).length
        });
        // Construir contexto narrativo com memória
        const narrativeContext = this.memory.generateNarrativeContext(currentScene.location || 'desconhecido', gameState.characterId);
        // Contexto para a IA
        const context = {
            scene: currentScene,
            character: {
        ...
        if (shouldGenerateEncounter) {
            try {
                encounter = await this.creatureGenerator.generateRandomEncounter(context.location, gameState.level > 5 ? 'experienced' : gameState.level > 10 ? 'veteran' : 'novice');
        ...
        if (choice?.consequences?.attributeChanges) {
            Object.entries(choice.consequences.attributeChanges).forEach(([attr, change]) => {
                attributeChanges[attr] = (attributeChanges[attr] || 0) + change;
            });
        }
    ...
    async generateIntelligentNarration(context) {
...
${(0, world_manifesto_1.getWorldManifestoPrompt)()}

${context.personalityPrompt}

PRINCÍPIOS CORE DO MUNDO:
${world_manifesto_1.WORLD_MANIFESTO.corePhilosophy}

${world_manifesto_1.WORLD_MANIFESTO.humanCondition}

${world_manifesto_1.WORLD_MANIFESTO.coreTruths}
...
Retorne JSON:
{
  ...
  "events": ["evento que pode acontecer em breve"]
    ...
    async generateNPCResponse(npc, playerAction, context) {
    ...
    calculateAttitudeChange(action, npcMemory) {
        let change = 0;
        // Ações positivas
    ...
    async generateIntelligentNPCResponse(npc, playerAction, context, npcMemory) {
        const personality = this.personalityManager.getCurrentPersonality();
...
Retorne JSON:
{
  ...
  "hints": ["dica sutil sobre lore ou quests"]
    ...
    async generateRandomEvent(location, playerLevel, gameState) {
        // 20% de chance de evento aleatório com criatura
        if (Math.random() > 0.2)
            return null;
        try {
            const creatureEncounter = await this.creatureGenerator.generateRandomEncounter(location, playerLevel > 5 ? 'experienced' : playerLevel > 10 ? 'veteran' : 'novice');
            return {
    ...
    getEncounterTitle(type) {
        ...
        return titles[type] || 'Evento Misterioso';
    }
    getCreatureEncounterTitle(danger) {
        const titles = {
        ...
        return titles[danger] || 'Encontro Misterioso';
    }
    generateCreatureChoices(creature) {
        const choices = [];
        // Opções pacíficas
    ...
    async adaptWorldToPlayer(gameState, playerChoices) {
        ...
        const aggressiveChoices = playerChoices.filter(choice => choice.toLowerCase().includes('atacar') ||
        ...
        const curiousChoices = playerChoices.filter(choice => choice.toLowerCase().includes('investigar') ||
    ...
    async generateDynamicQuest(playerLevel, location, playerBackground) {
        return await openai_service_1.openaiService.generateDynamicQuest(playerLevel, location, playerBackground);
    }
    /**
     ...
     */
    async generateTargetedCreatureEncounter(gameState, context) {
        ...
        try {
            ...
            if (spiritualAwareness < 7) {
                ...
                targetType = Math.random() < 0.5 ? creature_types_1.CreatureType.KAMI_MENORES : creature_types_1.CreatureType.TSUKUMOGAMI;
            }
            else if (spiritualAwareness < 10) {
                // Média percepção: criaturas emocionais
                targetType = Math.random() < 0.5 ? creature_types_1.CreatureType.YUREI : creature_types_1.CreatureType.MONONOKE;
            }
            else if (spiritualAwareness < 13) {
                // Alta percepção: criaturas transformadoras
                targetType = Math.random() < 0.5 ? creature_types_1.CreatureType.SUBSTITUTOS : creature_types_1.CreatureType.GHOULS;
            }
            else {
                // Percepção muito alta: criaturas alienígenas
                targetType = Math.random() < 0.5 ? creature_types_1.CreatureType.ENTIDADES_CONTATO : creature_types_1.CreatureType.YOKAI_TRADICIONAIS;
            }
    ...
    processCreatureResolution(creatureEncounter, resolutionType, characterEvolution, success = true) {
        // Registrar encontro na evolução do personagem
        characterEvolution.recordCreatureEncounter(creatureEncounter.type, success, `Resolução ${resolutionType} de encontro com ${creatureEncounter.type}`, success ? this.generateCreaturePower(creatureEncounter.type, resolutionType) : undefined);
    }
    // Gerar poder baseado no tipo de criatura e resolução
    generateCreaturePower(creatureType, resolutionType) {
        const powers = {
...
exports.GameMasterEngine = GameMasterEngine;
exports.gameMaster = new GameMasterEngine();
// Inicializar memória na criação
exports.gameMaster.initializeMemory();
